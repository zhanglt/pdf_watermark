...
private:
    ...
    void set_cpp_binding_options(void)
    {
        PDFCPP_TRY
        {
            m_PDFlib_api->PDF_set_option(p, "objorient=true");
            if (conv::do_conversion())
            {
                m_PDFlib_api->PDF_set_option(p, "binding={C++ conv}");
            ...
            else
            {
                ...
                {
                case sizeof(char):
                    m_PDFlib_api->PDF_set_option(p, "binding={C++ legacy}");
                    break;

                case utf16_wchar_t_size:
                    m_PDFlib_api->PDF_set_option(p, "apitextformat=utf16");
                    m_PDFlib_api->PDF_set_option(p, "binding=C++");
                    set_unicaplang_parameters();
                    break;

                case utf32_wchar_t_size:
                    m_PDFlib_api->PDF_set_option(p, "apitextformat=utf32");
                    m_PDFlib_api->PDF_set_option(p, "binding=C++");
                    set_unicaplang_parameters();
                    break;
...
#  endif // PDFCPP_DL
    ...
    {
        m_PDFlib_api->PDF_set_option(p, "unicaplang=true");
        m_PDFlib_api->PDF_set_option(p, "textformat=auto2");
        m_PDFlib_api->PDF_set_option(p, "hypertextformat=auto2");
        m_PDFlib_api->PDF_set_option(p, "hypertextencoding={}");
    }
